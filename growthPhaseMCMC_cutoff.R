...
logPrior <- function(cutPoints, nTime, startPoint, cutoff){
	augmentedCutPoints1 <- c(startPoint - 1, cutPoints[1:2], cutoff + 1)
	augmentedCutPoints2 <- c(cutoff, cutPoints[3], nTime)
	sanityCheck1 <- diff(augmentedCutPoints1)
	sanityCheck2 <- diff(augmentedCutPoints2)
	criterion <- min( sanityCheck1, sanityCheck2 )
	if( criterion <= 0 ){logP <- log(0)}else{
	logP <- -log(cutoff - startPoint) - log(cutoff - cutPoints[1]) - log(nTime - cutoff - 1)}
	return(logP)
}

simulateFromPrior <- function(nTime, startPoint, cutoff){
	cutPoints <- numeric(3)
	indexRange <- startPoint:(cutoff-1)
	if(length(indexRange) == 1){
		cutPoints[1] <- indexRange}else{
		cutPoints[1] <- sample(indexRange, 1)
	}
	indexRange <- (cutPoints[1] + 1):cutoff
	if(length(indexRange) == 1){
			cutPoints[2] = indexRange
		}else{
			cutPoints[2] <- sample(indexRange, 1)
	}
	indexRange <- (cutoff + 1):(nTime-1)
	if(length(indexRange) == 1){
...
localProposal <- function(cutPoints, nTime, mhPropRange, startPoint, cutoff){
	...
	augmentedCutPoints1 <- c(startPoint - 1, newState[1:2], cutoff + 1)
	augmentedCutPoints2 <- c(cutoff, newState[3], nTime)
	sanityCheck1 <- diff(augmentedCutPoints1)
	sanityCheck2 <- diff(augmentedCutPoints2)
	criterion <- min( sanityCheck1, sanityCheck2 )
	if(  criterion <= 0 ){	# not valid state!
		propRatio <- log(0)
	}else{
		propRatio <- 0
	}
...
singleLocalProposal <- function(cutPoints, nTime, mhSinglePropRange, startPoint, cutoff){
...
mhSampler <- function(myData, nIter, finalIterationPdf, modelVariance, mhPropRange, mhSinglePropRange, movesRange, startPoint, cutoff, sdValues, dName, timeScale, burn, showProgress){
	...
	nTime <- dim(myData)[1]
	if( sum((diff(c(startPoint, cutoff, nTime)) > 0)*1) != 2 ){stop("not valid parameters: nTime, startPoint, cutoff")}
	cutPoints <- array(data = NA, dim = c(nIter, 3))
	iter <- 1
	cutPoints[iter, ] <- simulateFromPrior(nTime = nTime, startPoint = startPoint, cutoff = cutoff)
	...
	lPosterior[iter] <- lValues[iter] + logPrior(cutPoints = cutPoints[iter, ], nTime = nTime, startPoint = startPoint, cutoff = cutoff) 
	mhRate <- 0
	mhRate2 <- 0
	mhRate3 <- 0
...
#			Move a: proposal from prior
			newCutPoints <- simulateFromPrior(nTime = nTime, startPoint = startPoint, cutoff = cutoff)
...
#			Move b: local random walk
			lpProp <- localProposal(cutPoints = cutPoints[iter, ] , nTime = nTime, mhPropRange = mhPropRange, startPoint = startPoint, cutoff = cutoff)
			newCutPoints <- lpProp$newState
			propLogL <- logLikelihood(myData = myData, cutPoints = newCutPoints,
					sdPerPoint = sdPerPoint, modelVariance = modelVariance, startPoint = startPoint)
			mhRatio <- propLogL - lValues[iter] + lpProp$propRatio + 
				logPrior(cutPoints = newCutPoints, nTime = nTime, startPoint = startPoint, cutoff = cutoff) - 
				logPrior(cutPoints = cutPoints[iter, ], nTime = nTime, startPoint = startPoint, cutoff = cutoff)
...
#			Move c: random walk to just one index
			lpProp <- singleLocalProposal(cutPoints = cutPoints[iter, ], nTime = nTime, 
					mhSinglePropRange = mhSinglePropRange, startPoint = startPoint, cutoff = cutoff)
			newCutPoints <- lpProp$newState
			propLogL <- logLikelihood(myData = myData, cutPoints = newCutPoints,
						sdPerPoint = sdPerPoint, modelVariance = modelVariance, startPoint = startPoint)
			mhRatio <- propLogL - lValues[iter] + lpProp$propRatio + 
		...
		if(iter %% 1000 == 0){
			...
			abline(v = cutoff, lty = 4, col = "gray")
...
#	burn = 2000, nIter = 5000,mhPropRange = 1, mhSinglePropRange = 50, getSDvalues = T, startPoint=54, timeScale = 1/6,
	...
	for (i in 1:n){
		...
		if( max( myData ) < blankThreshold ){
			...
			cutoff <- which( rowMeans(myData) >= yCutoff )[1]
			if( (cutoff < startPoint + 1)|( cutoff > dim(myData)[1] - 2 )|(is.na(cutoff)==TRUE) ){
				cat(paste0(" [WARNING] cutoff not achieved...  "))
				cutoff <- dim(myData)[1] - 2
			}
