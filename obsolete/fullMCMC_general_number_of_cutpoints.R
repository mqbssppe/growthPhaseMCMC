...
priorNumberOfCutpoints <- function(prior = "uniform", LRange){
	
	Lmax <- max(LRange)
	Lmin <- min(LRange)
	logPrior <- log(numeric(Lmax))
	if(prior == "uniform"){
		# discrete Uniform {1,..., Lmax}
		logPrior[Lmin:Lmax] <- rep(-log(Lmax - Lmin + 1), Lmax - Lmin + 1)
	}else{
	...
	logPrior <- exp(-(1:Lmax)^2)
	logPrior <- logPrior/sum(logPrior)
...
priorNumberOfCutpoints <- function(prior = "uniform", LRange){
	...
	logPrior <- log((logPrior1 + logPrior2 + logPrior3 + logPrior4)/4)
	names(logPrior) <- 1:Lmax
	return(logPrior)
...
updateNumberOfCutpoints <- function(cutPoints, nTime, startPoint, LRange, birthProbs){
	# this update propose cutpoint to be born with mean parameters deterministically equal to their observed mean.
...
#	L <- Lvalues[iter] <- max(LRange)
	...
	logPriorNPoints <- priorNumberOfCutpoints(prior = "poisson", LRange = LRange)
