...
p <- 0.15       # probability of same sign with previous                                ####
...
myRep <- rep(1,10)
pdf(file = "supplementary_fig1.pdf", width = 9, height = 4)
par(mfrow =  c(2,5), mar = c(3,3,1,1));
myVec <- c()
for(j in 0:9){
	ind <- which(Ncuts == j)
	myIndex = ind[myRep[j+1]]
	myVec <- c(myVec, myIndex)
	for(i in myIndex){ 
		matplot(cbind(myDataList[[1]][,i], myDataList[[2]][,i], myDataList[[3]][,i]), col = 2:4,type = "l", xlab = "t", ylab = "x", lty = 1)
		if(Ncuts[i] > 0){
			abline(v = cutPointsList[[i]][-(Ncuts[i]+1)], col = 1, lty = 2)
		}
		#legend("topleft", paste0(Ncuts[i], " cutpoints"))
	}
}
dev.off()
