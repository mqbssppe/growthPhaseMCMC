...
#		1st: summary plot
		...
		if(timeScale == 1){
			matplot(t(myMeanTimeSeries[subsetIndex,]), type = "l", col = myPal[unlist(x$NumberOfCutPoints_MAP)[subsetIndex]], lty = 1, xlab = "t", ylab = paste0("average ", ylab))
		}else{
			matplot(t(myMeanTimeSeries[subsetIndex,]), xaxt = "n", type = "l", col = myPal[unlist(x$NumberOfCutPoints_MAP)[subsetIndex]], lty = 1, xlab = "t", ylab = paste0("average ", ylab))
			axis(1, at=myPositions,labels=myLabels)
		}
		legend("topleft", col = c(0,myPal), lty = 1, c("MAP number of change-points:", paste0(1:nColors," (",as.numeric(table(unlist(x$NumberOfCutPoints_MAP)[subsetIndex]))," time-series)")))
